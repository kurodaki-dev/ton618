...
The registry's URL defaults to a placeholder (see `TON618_REGISTRY_URL` in `include/Version.hpp`) and can be pointed at any deployment — your own, or someone else's — without rebuilding, via the `TON618_REGISTRY_URL` environment variable:
...
L'URL du registre a une valeur par défaut de type placeholder (voir `TON618_REGISTRY_URL` dans `include/Version.hpp`) et peut être redirigée vers n'importe quel déploiement — le tien, ou celui de quelqu'un d'autre — sans recompiler, via la variable d'environnement `TON618_REGISTRY_URL` :
