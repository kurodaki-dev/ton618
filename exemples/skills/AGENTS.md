# TON618 — instructions for AI coding agents

Write, run, and debug TON618 (`.ton`) scripts — a small interpreted language (C++ runtime) with a distinctive `ton.` prefix, a built-in HTTP server, and a compact standard library. Read this whole file before writing or editing any `.ton` code; it's a dense reference, not a tutorial — more detail than you'll usually need in one script, but everything needed to write correct TON618 without consulting the full docs.
